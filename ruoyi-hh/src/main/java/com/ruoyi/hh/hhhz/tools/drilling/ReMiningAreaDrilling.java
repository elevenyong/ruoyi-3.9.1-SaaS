...
import com.ruoyi.hh.hhhz.domain.param.ReMiningAreaDrillingParam;
 ...
 */
@Component
public class ReMiningAreaDrilling {
...
*/
     ...
     * @return 施工数据
     */
    public ReMiningAreaDrillingParam calDrilling(ReMiningAreaDrillingParam param) {
        /*double lL,lR;
     ...
     * @return 验证孔位置
     */
    public ReMiningAreaDrillingParam preCal(ReMiningAreaDrillingParam param) {
        ...
        if (param.neicuo) {
            verifyPoint.setX(-param.lStart);
            verifyPoint.setY(((-param.lStart) * Math.tan(Math.PI - param.alf)) + ((param.h2 + param.hc + param.hg) / Math.cos(param.alf)));
            verifyPoint.setZ(0);
        ...
        borderLP.setX(ctlLP.getX() - (param.lEnd * Math.cos(param.alf)));
        borderLP.setY(ctlLP.getY() + (param.lEnd * Math.sin(param.alf)));
        System.out.println("左下边界点：" + borderLP);
        Point borderRP = new Point();
        borderRP.setX(ctlRP.getX() + (param.lEnd * Math.cos(param.alf)));
        borderRP.setY(ctlRP.getY() - (param.lEnd * Math.sin(param.alf)));
        ...
        // 计算控制范围
        param.lL = param.lEnd + (param.wm / (2 * Math.cos(param.alf))) + (param.hc / Math.tan(roL));
        param.lR = param.lEnd + (param.wm / (2 * Math.cos(param.alf))) + (param.hc / Math.tan(roR));
     ...
     * 计算正方形布孔
     */
    private ReMiningAreaDrillingParam calDrilling4(ReMiningAreaDrillingParam param, int lNum/*, double lL, double lR, double s4, double alf*/) {
        // 正方形布置
        param.numL4 = (int) (Math.floor(param.lL / param.s4) + 1);
        param.numR4 = (int) (Math.floor(param.lR / param.s4) + 1);
     ...
     * 计算三角布局
     */
    private ReMiningAreaDrillingParam calDrilling3(ReMiningAreaDrillingParam param, int lNum) {
        // 三角布置
        param.numL3 = Math.floor(((param.lL - (param.s3 / 2)) / param.s3) + 2);
        param.numR3 = Math.floor(((param.lR - (param.s3 / 2)) / param.s3) + 2);
     ...
     * @param point 出煤点
     */
    public NormalDrillingData calData(ReMiningAreaDrillingParam param, Point point) {
        // 钻孔长度

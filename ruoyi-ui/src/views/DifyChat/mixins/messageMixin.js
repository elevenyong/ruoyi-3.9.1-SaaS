...
export default {
  ...
  data() {
    return {
      ...
      // Dify 配置
      difyConfig: {
        baseURL: 'http://192.168.0.20:9988/dev-api/ai',
        apiKey: getToken(),
  ...
  methods: {
    ...
    async handleStreamMessage(data) {
      ...
      if (String(data.data).search("[DONE]")===-1){
        data.data=JSON.parse(data.data.substring(5))
      }
      ...
      switch (data.type) {
        ...
        case 'delta':
          ...
          if (true) {
            ...
            console.log("choices",data.data.choices[0].delta)
            let deltaContent = data.data.choices[0].delta.content
            // 普通内容，直接添加

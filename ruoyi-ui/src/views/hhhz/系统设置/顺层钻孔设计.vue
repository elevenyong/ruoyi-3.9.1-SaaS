      ...
      <el-col :span="6" class="card-box">
        <el-card style="height: 85vh">
          ...
          <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" label-width="188px">
            ...
            <el-form-item label="顺槽方位角" prop="belt">
              <el-input
                v-model="queryParams.belt"
                placeholder="顺槽方位角"
              >
                <template slot="append">°</template>
              </el-input>
            </el-form-item>
            <el-form-item label="顺槽宽" prop="wm">
              <el-input
            ...
            <el-form-item label="顺槽墙高1" prop="hm1">
              <el-input
                v-model="queryParams.hm1"
                placeholder="顺槽墙高1"
              >
                <template slot="append">m</template>
              </el-input>
            </el-form-item>
            <el-form-item label="顺槽墙高2" prop="hm2">
              <el-input
                v-model="queryParams.hm2"
                placeholder="顺槽墙高2"
              >
                <template slot="append">°</template>
              </el-input>
            ...
            <el-form-item label="顺槽长度" prop="scLen">
              <el-input
                v-model="queryParams.scLen"
                placeholder="顺槽长度"
              >
                <template slot="append">m</template>
              </el-input>
            </el-form-item>
            <el-form-item label="顺槽类型" prop="upBedding">
              <el-select v-model="queryParams.upBedding" placeholder="请选择">
                <el-option
                  v-for="item in upBeddings"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value">
                </el-option>
...
<script>
import { drillingBedding } from '@/api/hhhz/drillingdes'
...
export default {
  ...
  data() {
    return {
      // 内外错
      upBeddings: [
        { value: true, label: '上顺槽' },
        { value: false, label: '下顺槽' }
      ],
      // 遮罩层
      loading: false,
      // 选中数组
      ...
      queryParams: {
        ...
        'lx': 70, //抽采控制范围
        'scLen': 20, //顺槽长度
        'upBedding': false //true:上顺槽|false:下顺槽
      },
      // 表单参数
      form: {},
      // 表单校验
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      drillingBedding(this.queryParams).then(response => {

<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="冲孔数量(个)" align="center" prop="boreholeId"/>
      <el-table-column label="冲孔段长度(m)" align="center" prop=""/>
      <el-table-column
        label="应冲出煤量(m³)"
        align="center"
        prop="boreholeId"
      ...
      <!-- <el-table-column label="孔号ID" align="center" prop="boreholeId" />
            ...
            <el-table-column label="开始冲孔时间" align="center" prop="timeBeginFlushing" width="180">
                <template slot-scope="scope">
                    <span>{{ parseTime(scope.row.timeBeginFlushing, '{y}-{m}-{d}') }}</span>
                </template>
            </el-table-column>
            <el-table-column label="冲孔结束时间" align="center" prop="timeEndFlushing" width="180">
                <template slot-scope="scope">
                    <span>{{ parseTime(scope.row.timeEndFlushing, '{y}-{m}-{d}') }}</span>
                </template>
            </el-table-column>
...
<script>
import { gChongkongmeiliang } from '@/api/hhhz/18ckmlwcqk'
import { gRoadWayList } from '@/api/hhhz/zuankong'

export default {
  name: 'TActualFlushing',
  data() {
    return {
      // 钻孔地点
      queryRoadWay: [],
  ...
  methods: {
    ...
    getList() {
      ...
      gChongkongmeiliang(this.queryParams).then((response) => {
        this.tActualFlushingList = response.rows
    ...
    handleExport() {
      this.download(
        'tActualFlushing/exportFlushingStatistics',
        {},
        `冲孔煤量完成情况_${new Date().getTime()}.xlsx`
      )
    }
  }
}

...
export default {
  ...
  methods: {
    ...
    createPDF(date, tbody) {
      ...
      this.doc.text("瓦斯抽采日报表", 150, 20, null, null, "center");
 ...
 */ let headers = this.createHeaders([
    ...
    getList() {
      ...
      gWasiribaoL(this.queryParams).then((response) => {
        ...
        let ds = this.queryParams.time.split("-");
        PDFObject.embed(
    ...
    handleQuery() {
      ...
      /* if (this.selectDate) {
                this.queryParams.time = this.selectDate.toLocaleString().replaceAll("/", "-").subString(0, 10);
            } */
      this.getList();
    },
    /** 重置按钮操作 */
